...
class TestRemoveMonFromCluster(ManageTest):
    ...
    def test_remove_mon_pod_from_cluster(self):
        ...
        """
        list_mons = self.cluster.get_mons_from_cluster()
        ...
        self.cluster.cluster_health_check(timeout=0)
        self.cluster.remove_mon_from_cluster()
        assert self.verify_mon_pod_up(), (
            "Mon pods are not up and running state"
        )
        self.cluster.cluster_health_check(timeout=60)

    @retry(CephHealthException, 8, 3, 1)
    def verify_mon_pod_up(self):
        """
        ...
        self.cluster.cluster_health_check(timeout=3)
        ret = self.pod.wait_for_resource(
